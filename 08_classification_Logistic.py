...
### 1. Establishes relation between X and y of permafrost data through Logistic Classification
### 2. Saves the predicted map as tiff<br/>  
### 3. Prepares a side by side plot of base map and predicted permafrost map<br/>
### 4. Plots an elevation band 
...
def outMap(makeTif):
    ...
    if makeTif==1:
        ...
        with rio.Env():
            profile = src.profile
            with rasterio.open("../07_Data/01_Raster/02_Out_Maps/07_LogR.tif", 'w', **profile) as dst:
                dst.write(yPredMap,1)
    print('Map saved at:', os.path.realpath("../07_Data/01_Raster/02_Out_Maps/07_LogR.tif"))
...
def plot1():
    ...
    ax2.set_title('Predicted Map [LogR]', fontsize = 14)
    # ax2.set_yticklabels([])
    # ax2.set_yticks([])
    ...
    # [6.3] save fig
    plt.savefig("../05_Images/03_PredMaps/07_LogR.png", bbox_inches='tight', facecolor='w')
    plt.close()
    print('Plot saved at:', os.path.realpath("../05_Images/03_PredMaps/07_LogR.png"))
plot1()
...
def plot2():
    ...
    # 5 save figure
    plt.savefig("../05_Images/04_Elev_Bands_Pred/07_LogR.png", facecolor='w', bbox_inches='tight')
    plt.close()
    print('Plot saved at:', os.path.realpath("../05_Images/04_Elev_Bands_Pred/07_LogR.png"))
